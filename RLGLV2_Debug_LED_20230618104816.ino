 ...
 * Last updated:	13 September 2022/ 21 January 2023
 * Project Name:	Red-Light Green-Light
*/

// constants won't change. They're used here to set pin numbers:
const int gamePowerPin = A1;    // the number of the game on/off Pin
...
void loop() {
  ...
  // Wait for Game Switch to Be Turned on
  ...
  delay(1000);
  if(gamePowerPin == HIGH) {                            // While Game Pin Off....
    for (int i = 0; i < NumberOfLEDS; i++) {            // Turn Off All LED Relays (Leaves on normal light operations)
      digitalWrite(LEDPins[i], LOW);
    }
    P1Wins = 0;                                         // Reset All Game Variables
    P2Wins = 0;
    P1Distance = 0;
    P2Distance = 0;
    didP1Lose = false;
  ...
  if(gamePowerPin == LOW) {                              // Is Game turned on?
    digitalWrite(LEDPins[0], HIGH);                      // Turn on Game LED
    ...
    /* if (digitalRead(gameSelectPin) == 1) {                             // Game 1 Selected
     ...
     while(digitalRead(switchPin) == LOW) {
      ...
      //Turn off All LEDs (Keep Game Power Light On)
      for (int i = 0; i < NumberOfLEDS; i++) {
      ...
      for(int i = 0; i < P1Wins || i < P2Wins; i++) {
	      ...
	      if(i < P2Wins) {
  	      digitalWrite(LEDPins, LOW);         // P2 Yellow On
	      }
  	    delay(100);
    ...
    if(digitalRead(gameSelectPin) == 0) {                        // Game 2 Selected    
